function bootApp() {
  ...
  Store.set = function(key, val){
    ...
    // v0.38：恢复 UserDataAPI 持久化挂钩（原为死代码）；AI 配置按档案独立文件存取
    try { if (UserDataAPI && UserDataAPI.syncKey) UserDataAPI.syncKey(key, val); } catch(e){}
  ...
  function renderWordCard(word) {
    ...
    document.getElementById('wordAiExBtn').onclick = function() {
      var btn = this;
  ...
  function renderExerciseNav() {
    ...
    html += '<button class="sbtn' + (exerciseState.type==='blank'?' sel':'') + '" data-ex-type="blank">🖊️填空默写</button>';
    ...
    navEl.querySelectorAll('[data-ex-type]').forEach(function(btn){
      btn.onclick = function(){
        ...
        // v0.38：先 showPlaceholder（内部 hideAllViews 会清 .sel）再 renderSidebar，保住选中高亮
  ...
  function renderExArtNav() {
    var html = '';
    html += '<button class="sbtn" id="exBackBtn" style="margin-bottom:4px">🔙返回练习类型</button>';
  ...
  async function callAI(cfg, prompt, timeoutMs) {
    ...
    try {
      ...
      if (!proxyResp.ok) {
        ...
        try { var ej = JSON.parse(errText); if (ej && ej.error) errText = (typeof ej.error === 'string') ? ej.error : (ej.error.message || JSON.stringify(ej.error)); } catch (e2) {}
